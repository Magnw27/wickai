...
export function WickAIChat() {
  ...
  const ready = status === "ready" || status === undefined || status === "error";
  ...
  const submit = (event: React.FormEvent) => {
    ...
    sendMessage({ text }, { body: { model }, onError: () => undefined });
    setInput("");
  };
  ...
  const copyMessage = async (id: string, text: string) => {
    ...
    } catch { /* ignore clipboard restrictions */ }
  };
    ...
    <main className="wick-shell flex min-h-dvh text-zinc-100">
      ...
      <aside className={`glass fixed inset-y-0 left-0 z-40 w-[292px] p-3.5 transition-transform duration-300 lg:static lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-full flex-col">
          ...
          <button onClick={startNewChat} className="mb-3 flex items-center gap-2 rounded-xl bg-white px-3.5 py-3 text-sm font-semibold text-black shadow-lg shadow-black/20 transition hover:-translate-y-0.5 hover:bg-zinc-100">
          ...
          <div className="wick-scrollbar flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto pr-1">
            {history.length === 0 ? (
              <div className="rounded-xl border border-dashed border-white/[.07] p-4 text-xs leading-5 text-zinc-600">Your recent conversations will appear here.</div>
            ) : history.map((chat) => (
              <div key={chat.id} className="group flex items-center gap-1">
                <button onClick={() => openChat(chat)} className={`min-w-0 flex-1 rounded-xl px-3 py-2.5 text-left text-sm transition hover:bg-white/[.05] ${chat.id === chatId ? "bg-white/[.055] text-zinc-200" : "text-zinc-500"}`}>
                  ...
                  <span className="mt-0.5 block text-[10px] text-zinc-700">{new Date(chat.updatedAt).toLocaleDateString()}</span>
                </button>
      ...
      <section className="relative flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex h-[62px] items-center justify-between border-b border-white/[.055] bg-black/15 px-3 backdrop-blur-2xl sm:px-5 lg:px-7">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <label className="flex items-center gap-2 rounded-xl border border-white/[.07] bg-white/[.035] px-2.5 py-2 text-xs text-zinc-300 ring-1 ring-black/10">
